...
foreach ($mapInfo in $mapsToProcess) {
    ...
    # Use the first/largest OBJ file if multiple were extracted
    ...
    try {
        ...
        Write-Host "  [OK] Compiled: $outputGmdag ($('{0:N0}' -f $gmdagSize) bytes)" -ForegroundColor Green
        $compileOk++
    }
    catch {
