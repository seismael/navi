...
- **Mesh Repair Standard:** Datasets whose GLB scenes store geometry in local space with scene-graph scale transforms (e.g. ReplicaCAD Baked Lighting) MUST be compiled with `--repair`, which applies scene-graph transforms to world space, merges vertices, fills holes, and fixes normals before voxelization. The `repair_mesh` pipeline uses `trimesh.Scene.dump()` to produce correct world-space geometry.
- **Observation Quality Gate:** All compilation and corpus-promotion pipelines MUST run `qualify-gmdag` after compilation to validate observation quality via real CUDA ray casting. Scenes that FAIL the observation quality gate (0 viable spawn candidates or best starvation ≥ 70%) MUST be rejected and removed. The `qualify-gmdag` CLI command probes a 5×7×5 candidate grid with 48×12 ray directions and scores candidates using the same `_observation_profile` and `_spawn_candidate_score` functions used at runtime spawn selection.
