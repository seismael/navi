...
def train_ppo(
    sub: str = typer.Option(None, help="ZMQ SUB address"),
    pub: str = typer.Option(None, help="ZMQ PUB bind address for telemetry"),
    step_endpoint: str = typer.Option(
...
def train_sequential(
    ...
    actors: int = typer.Option(1, help="Number of parallel environments"),
    total_steps: int = typer.Option(1000000, help="Total environment steps"),
    ...
    ppo_epochs: int = typer.Option(2, help="PPO epochs per update"),
    minibatch_size: int = typer.Option(32, help="Minibatch size"),
    bptt_len: int = typer.Option(16, help="BPTT sequence length"),
    clip_ratio: float = typer.Option(0.2, help="PPO clip ratio"),
    gamma: float = typer.Option(0.99, help="Discount factor"),
    gae_lambda: float = typer.Option(0.95, help="GAE lambda"),
    entropy_coeff: float = typer.Option(0.01, help="Entropy bonus coefficient"),
    ...
    env_pub: str = typer.Option(None, help="Environment PUB bind address"),
    env_rep: str = typer.Option(None, help="Environment REP bind address"),
    ...
    config = ActorConfig(
        sub_address=env_pub or default_config.sub_address,
        pub_address=actor_pub or default_config.pub_address,
        mode="step",
        step_endpoint=env_rep or default_config.step_endpoint,
        azimuth_bins=azimuth_bins,
        elevation_bins=elevation_bins,
        n_actors=actors,
        embedding_dim=embedding_dim,
    ...
    # Extract scene paths from manifest
    scenes: list[str] = []
    ...
    env_config = EnvironmentConfig(
        pub_address=config.sub_address.replace("localhost", "*"), # Bind address
        rep_address=config.step_endpoint.replace("localhost", "*"),
        mode="step",
        training_mode=True,
        n_actors=actors,
        backend=backend,
