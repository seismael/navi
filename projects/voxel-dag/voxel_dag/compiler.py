...
class SdfComputer(ABC):
    """Abstract base class for SDF computation (Open/Closed Principle)."""
    @abstractmethod
    def compute(
        self,
        vertices: np.ndarray,
...
class EikonalSdfComputer(SdfComputer):
    """Computes a cubic dense unsigned distance field using triangle-mesh distances + Eikonal propagation."""
    ...
    def _solve_eikonal(self, a: float, b: float, c: float, h: float) -> float:
            ...
            for iz in range(iz_start, iz_end + 1):
                ...
                for iy in range(iy_start, iy_end + 1):
                    ...
                    for ix in range(ix_start, ix_end + 1):
                        ...
                        d2 = self._point_triangle_distance_sq(px, py, pz, v0, v1, v2)
                        d = math.sqrt(d2)
                        if d < field[iz, iy, ix]:
                            field[iz, iy, ix] = d
            ...
            for iz in zr:
                for iy in yr:
                    for ix in xr:
                        ...
                        updated = self._solve_eikonal(ax, ay, az, h)
...
class DagCompressor(ABC):
    """Abstract base class for DAG compression."""
    @abstractmethod
    def compress(self, grid: np.ndarray, resolution: int, cell_size: float = 0.0) -> np.ndarray:
        pass


class SvoDagCompressor(DagCompressor):
    """Compresses a dense grid to a DAG payload."""

    def _float_to_half_bits(self, value: float) -> int:
        return int(np.asarray([value], dtype=np.float16).view(np.uint16)[0])

    def _leaf_distance_from_block(self, block: np.ndarray) -> float:
        leaf_distance = float(block.min()) if block.size > 0 else 0.1
        # Snap to zero if within reasonable proximity to surface to ensure hits
        if leaf_distance < 0.08:
            return 0.0
        return max(0.0, min(leaf_distance, 30.0))

    def _leaf_signature_from_block(self, block: np.ndarray, semantic_block: np.ndarray | None = None) -> bytes:
        leaf_distance = self._leaf_distance_from_block(block)
        semantic = 0
        if semantic_block is not None and semantic_block.size > 0:
            # Simple majority vote or just take the first
            semantic = int(semantic_block.flat[0])
        return b"L" + struct.pack("<HH", self._float_to_half_bits(leaf_distance), semantic)

    def _leaf_node_from_signature(self, signature: bytes) -> np.uint64:
    ...
    def _build_signature_tree(
        self,
        block: np.ndarray,
        semantic_block: np.ndarray | None,
        specs: dict[bytes, _LeafSpec | _InternalSpec],
        cell_size: float = 0.0,
    ) -> bytes:
        if block.size == 0:
            leaf_signature = self._leaf_signature_from_block(np.array([0.1], dtype=np.float32), None)
            specs.setdefault(
                leaf_signature,
                _LeafSpec(
                    signature=leaf_signature,
                    node_word=int(self._leaf_node_from_signature(leaf_signature)),
                ),
            )
        ...
        if min(block.shape) <= 1 or bool(
            ...
            leaf_signature = self._leaf_signature_from_block(block, semantic_block)
            specs.setdefault(
                leaf_signature,
                _LeafSpec(
                    signature=leaf_signature,
        ...
        for octant in range(8):
            ...
            child_semantic_block = None
            if semantic_block is not None:
                child_semantic_block = semantic_block[
                    z_start : (block.shape[0] if octant & 4 else z_half),
                    y_start : (block.shape[1] if octant & 2 else y_half),
                    x_start : (block.shape[2] if octant & 1 else x_half),
                ]
            ...
            child_signatures.append(self._build_signature_tree(child_block, child_semantic_block, specs, cell_size))
            mask |= 1 << octant
        ...
        signature = self._encode_internal_signature(
            mask=mask,
    ...
    def _emit_signature(
        ...
        for child_signature in spec.unique_child_signatures:
            child_indices.append(
                self._emit_signature(child_signature, specs, dag_pool, emitted_indices)
            )
    ...
    def compress(self, grid: np.ndarray, resolution: int, cell_size: float = 0.0, semantic_grid: np.ndarray | None = None) -> np.ndarray:
        res = _next_power_of_two(resolution)
        ...
        if semantic_grid is not None:
            dense_semantic = np.asarray(semantic_grid, dtype=np.int32)
            if dense_semantic.shape != (res, res, res):
                dense_semantic = np.reshape(dense_semantic, (res, res, res)).astype(np.int32, copy=False)

        if dense.size == 0:
            return np.array(
                [np.uint64((1 << 63) | self._float_to_half_bits(0.1))], dtype=np.uint64
            )

        specs: dict[bytes, _LeafSpec | _InternalSpec] = {}
        root_signature = self._build_signature_tree(dense, dense_semantic, specs, cell_size=cell_size)
        dag_pool: list[np.uint64] = []
        self._emit_signature(root_signature, specs, dag_pool, emitted_indices={})
        return np.asarray(dag_pool, dtype=np.uint64)


class GmdagWriter(ABC):
...
class BinaryGmdagWriter(GmdagWriter):
    """Writes the canonical binary .gmdag format."""
...
def compute_dense_sdf(
    ...
    *,
    ...
    """Compatibility wrapper for EikonalSdfComputer."""
    computer = EikonalSdfComputer()
    return computer.compute(vertices, indices, bbox_min, bbox_max, resolution, padding)


def compress_to_dag(
    grid: np.ndarray, resolution: int, cell_size: float = 0.0
) -> np.ndarray:
    """Compatibility wrapper for SvoDagCompressor."""
    compressor = SvoDagCompressor()
    return compressor.compress(grid, resolution, cell_size)
...
def write_gmdag(
    ...
    """Compatibility wrapper for BinaryGmdagWriter."""
    writer = BinaryGmdagWriter()
    writer.write(path, dag, resolution, bbox_min, voxel_size)
