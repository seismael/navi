...
- During canonical PPO optimizer windows, the trainer may re-publish the last rendered `DistanceMatrix` at a coarse cadence to keep the passive dashboard responsive.
- These heartbeat frames are diagnostic-only and do not represent new environment steps or new rollout state.
...
The current PPO update pass now also treats minibatch assembly as part of the
hot path: multi-actor rollout tensors should be stacked once per PPO update and
reused across epochs, and minibatch shuffle indices should stay on the same
device as the sampled tensors to avoid incidental host-side indexing overhead.

### 2.4 TSDF-Derived Runtime Work

`TSDF.md` proposes three different classes of changes, and the repository does
not treat them equally:

1. accepted immediate work: align the `torch-sdf` CUDA kernel horizon with the
  configured environment horizon so runtime tracing and observation
  normalisation share one distance contract
2. accepted next-step work: environment-side starvation/proximity reward terms
  computed from the existing batched depth/validity tensors
3. benchmark-gated experimental work: compiler-side truncation metadata,
  truncated-distance storage, and Morton/Z-order DAG layout changes

The canonical runtime now includes the first two. Compiler/storage changes are
not canonical until they beat the current path on file size, load cost, and
end-to-end training throughput.

### 2.2 Diagnostic Reference Paths

- `SdfDagBackend`: the active compiled-scene runtime and performance surface.
